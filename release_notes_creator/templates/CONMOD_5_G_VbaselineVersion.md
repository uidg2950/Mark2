...
- MIPI switch is not supported on HW samples earlier than B2.

Software Limitation:

- ESO APParmour adaptation required for the following files:

 */sys/power/wakeup_count*
 */opt/cust/init/secure/chroot/proc/board/toggle_nad_lpm*

That's in order to suspend mode works properly.

**NOTE:** *Action is on ESO side, but still effective for our internal fully integrated builds.*


- In case SW loading is not executed OK, be aware that “no free space” on /data partition might be the reason.
  Please open an ADB shell and remove the IPACM log file:

  *rm -f /data/ipacm_log.txt*
  *sync*

and try SWL again

- Fix for CVE-2022-2503
  dm verity: set DM_TARGET_IMMUTABLE feature flag The device-mapper framework provides a mechanism to mark targets as immutable
  (and hence fail table reloads that try to change the target type). Add the DM_TARGET_IMMUTABLE flag to the dm-verity target's feature flags
  to prevent switching the verity target with a different target type.
